...
require 'nginx_alpine_digest="$(retry 5 get_nginx_digest)"'
require 'scriptomatic_main="$(retry 5 get_scriptomatic_sha)"'
require 'toolset_release="$(retry 5 get_toolset_release)"'
require 'toolset_installer_sha256="$(retry 5 get_installer_sha)"'
require 'current_nginx_alpine_digest="$(retry 5 get_nginx_digest)"'
require 'current_scriptomatic_main="$(retry 5 get_scriptomatic_sha)"'
require 'current_toolset_release="$(retry 5 get_toolset_release)"'
require 'current_toolset_installer_sha256="$(retry 5 get_installer_sha)"'
...
require 'Toolset installer content changed during release.'

retry_blocks="$(grep -Fc 'retry() {' "$workflow")"
[[ "$retry_blocks" -eq 2 ]] || fail "expected retry helper in snapshot and revalidation steps, found $retry_blocks"
